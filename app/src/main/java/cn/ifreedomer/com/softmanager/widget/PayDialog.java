...
public class PayDialog extends Dialog implements View.OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_pay:
                dismiss();
                ServiceManager.getPayInfo(HardwareUtil.getImei()).subscribeOn(Schedulers.io()).subscribe(respResult -> {
                    if (respResult.getResultCode() != RespResult.SUCCESS) {
                        getOwnerActivity().runOnUiThread(() -> Toast.makeText(getContext(), respResult.getMsg(), Toast.LENGTH_SHORT).show());
                        return;
                    }
                    pay(respResult);
                }, throwable -> {
    ...
    private void pay(RespResult<PayInfo> s) {
