...
import cn.ifreedomer.com.softmanager.adapter.ContentProviderAdapter;
...
public class ContentProviderFragment extends Fragment implements TabLayout.OnTabSelectedListener {
    ...
    private void initAllFragment() {
        ...
        ContentProviderAdapter systemAdapter = new ContentProviderAdapter(getActivity(), R.layout.item_common_recycle, systemApps);
        ...
        systemAdapter.setOnItemClickListener(new MultiItemTypeAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, RecyclerView.ViewHolder holder, int position) {
                if (systemApps == null || systemApps.size() == 0 || systemApps.get(position).getContentProviderList() == null || systemApps.get(position).getContentProviderList().size() == 0) {
                    Toast.makeText(getActivity(), getString(R.string.no_content), Toast.LENGTH_SHORT).show();
                    return;
                }
